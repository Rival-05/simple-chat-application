...
function messageHandler(ws, message) {
    if (message.type == incomingMessages_1.SupportedMessage.JOIN_ROOM) {
        const payload = message.payload;
        userManager.addUser(payload.name, payload.userId, payload.roomId, ws);
    }
    if (message.type == incomingMessages_1.SupportedMessage.SEND_MESSAGE) {
        const payload = message.payload;
    ...
    if (message.type == incomingMessages_1.SupportedMessage.UPVOTE_MESSAGE) {
        const payload = message.payload;
